...
export const createPost = asyncHandler(async (req, res, next) => {
  try {
    ...
    const hasPoll = Boolean(pollData && pollData.options && pollData.options.filter(option => option?.trim()).length >= 2);

    if (!content && !image && !hasPoll) {
      return next(new ApiError(400, 'Post content, image, or poll is required'));
    }
    ...
    if (hasPoll) {
      const pollOptions = pollData.options.filter(option => option?.trim()).map(option => ({
        text: option,
        votes: []
      }));
    ...
    if (error.name === 'ValidationError') {
      const message = Object.values(error.errors).map(({ message }) => message).join(', ');
      return next(new ApiError(400, message || 'Invalid post data'));
    }
